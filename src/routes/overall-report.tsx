...
function marksAndPercentage(value: number | null, total: number) {
  ...
  return { marks: Math.round(((percentage / 100) * total) * 10) / 10, percentage: Math.round(percentage * 10) / 10 };
}

function studentPercentage(student: StudentReport) {
  const values = ([ [student.ica, ASSESSMENT_TOTALS.ICA], [student.mca, ASSESSMENT_TOTALS.MCA], [student.fca, ASSESSMENT_TOTALS.FCA] ] as const)
...
function studentRank(student: StudentReport | undefined, students: StudentReport[]) {
  ...
  const peers = students
    .filter((s) => s.school_id === student.school_id && s.class === student.class && s.division === student.division)
...
function OverallReport() {
  ...
  const buildReport = () => {
    ...
    const body = (() => {
      if (mode === "student") return studentHtml(selectedStudent, students, studentRank(selectedStudent, students));
      if (mode === "school") {
  ...
  return (
    <main className="mx-auto max-w-7xl space-y-6 px-4 py-8 sm:px-6">
      ...
      <ReportPreview mode={mode} reports={reports} classReports={classReports} students={visibleStudents} selectedSchool={selectedSchool?.name} selectedStudent={selectedStudent} allStudents={students} />
    </main>
  );
}

function ReportPreview({ mode, reports, classReports, students, selectedSchool, selectedStudent, allStudents }: { mode: Mode; reports: ReturnType<typeof buildSchoolReports>; classReports: ReturnType<typeof buildClassReports>; students: StudentReport[]; selectedSchool?: string; selectedStudent?: StudentReport; allStudents: StudentReport[] }) {
  if (mode === "student") return <StudentPreview student={selectedStudent} allStudents={allStudents} />;
...
function StudentPreview({ student, allStudents }: { student?: StudentReport; allStudents: StudentReport[] }) { if (!student) return <Card className="p-8 text-center text-muted-foreground">Choose a student to preview the marksheet.</Card>; const scores = [["ICA", student.ica, ASSESSMENT_TOTALS.ICA], ["MCA", student.mca, ASSESSMENT_TOTALS.MCA], ["FCA", student.fca, ASSESSMENT_TOTALS.FCA]] as const; const converted = scores.map(([name, value, max]) => [name, marksAndPercentage(value, max), max] as const); const total = converted.reduce((sum, [, result]) => sum + (result.marks ?? 0), 0); const rank = studentRank(student, allStudents); return <Card className="relative overflow-hidden rounded-2xl border-2 border-primary/40 bg-gradient-to-br from-card via-card to-primary/5 p-6 shadow-elegant before:absolute before:inset-2 before:rounded-xl before:border before:border-primary/15 before:content-[""]"><div className="relative z-10 flex items-start gap-3 rounded-xl border border-primary/20 border-b-4 border-b-destructive bg-gradient-to-r from-primary/5 via-card to-warm/10 p-4 pb-4"><img src="/reap-logo.png" alt="REAP logo" className="h-14 w-28 object-contain object-left" /><div><h2 className="font-display text-2xl font-bold">STUDENT PERFORMANCE MARKSHEET</h2><p className="text-sm text-muted-foreground">Academic Year: 2026–27 · SchoolRise</p></div></div><div className="relative z-10 mt-6 grid gap-3 rounded-xl border border-cyan-200/70 bg-cyan-50/30 p-3 sm:grid-cols-2"><Info label="School Name" value={student.school_name} /><Info label="Student Name" value={student.name} /><Info label="Student ID" value={student.student_code} /><Info label="Roll No." value={student.roll_number} /><Info label="Class" value={student.class} /><Info label="Section / Division" value={student.division} /></div><h3 className="relative z-10 mt-7 rounded-lg border border-violet-200 border-l-4 border-l-violet-500 bg-violet-50/50 px-3 py-2 font-display text-lg font-semibold text-primary">ACADEMIC PERFORMANCE</h3><table className="relative z-10 mt-3 w-full overflow-hidden rounded-xl border-2 border-primary/20 border-collapse text-sm"><thead><tr><th className="border border-border bg-primary px-3 py-2 text-left text-primary-foreground">Sr. No.</th><th className="border border-border bg-primary px-3 py-2 text-left text-primary-foreground">Subject / Assessment</th><th className="border border-border bg-primary px-3 py-2 text-right text-primary-foreground">Total Marks</th><th className="border border-border bg-primary px-3 py-2 text-right text-primary-foreground">Marks Obtained</th><th className="border border-border bg-primary px-3 py-2 text-right text-primary-foreground">Percentage</th></tr></thead><tbody>{converted.map(([name, result, max], index) => <tr key={name}><td className="border border-border px-3 py-2">{index + 1}</td><td className="border border-border px-3 py-2">{name}</td><td className="border border-border px-3 py-2 text-right">{max}</td><td className="border border-border px-3 py-2 text-right">{result.marks ?? "—"}</td><td className="border border-border px-3 py-2 text-right">{result.percentage == null ? "—" : `${result.percentage}%`}</td></tr>)}</tbody></table><div className="relative z-10 mt-5 grid gap-3 rounded-xl border border-orange-200 bg-orange-50/30 p-3 sm:grid-cols-5"><Metric label="Total Marks" value="50" /><Metric label="Obtained" value={total} /><Metric label="Percentage" value={`${studentPercentage(student)}%`} /><Metric label="Overall Grade" value={grade(studentPercentage(student))} /><Metric label="Status" value={performanceStatus(studentPercentage(student))} /></div><p className="relative z-10 mt-6 rounded-xl border border-emerald-200 border-l-4 border-l-emerald-500 bg-emerald-50/60 p-4 text-sm"><b>Overall Performance:</b> {performanceStatus(studentPercentage(student))}<br /><b>Class / Section Rank:</b> {rank ? `${rank}` : "—"}</p></Card>; }
...
function studentHtml(student: StudentReport | undefined, allStudents: StudentReport[], rank: number | null) { if (!student) return "<h2>INDIVIDUAL STUDENT REPORT</h2><p>No student selected.</p>"; const scores = [["ICA", student.ica, ASSESSMENT_TOTALS.ICA], ["MCA", student.mca, ASSESSMENT_TOTALS.MCA], ["FCA", student.fca, ASSESSMENT_TOTALS.FCA]] as const; const rows = scores.map(([name, score, total], i) => { const result = marksAndPercentage(score, total); return [i + 1, name, total, result.marks ?? "—", result.percentage == null ? "—" : `${result.percentage}%`]; }); const obtained = scores.reduce((sum, [, score, total]) => sum + (marksAndPercentage(score, total).marks ?? 0), 0); return `<h2>STUDENT DETAILS</h2><div class="grid"><div class="field"><b>Student Name:</b>${escapeHtml(student.name)}</div><div class="field"><b>Student ID:</b>${escapeHtml(student.student_code)}</div><div class="field"><b>School Name:</b>${escapeHtml(student.school_name)}</div><div class="field"><b>Roll No.:</b>${escapeHtml(student.roll_number)}</div><div class="field"><b>Class:</b>${escapeHtml(student.class)}</div><div class="field"><b>Section:</b>${escapeHtml(student.division)}</div></div><h2>ACADEMIC PERFORMANCE</h2>${table(["Sr. No.", "Subject / Assessment", "Total Marks", "Marks Obtained", "Percentage"], rows)}<h2>PERFORMANCE SUMMARY</h2>${table(["Total Marks", "Obtained Marks", "Percentage", "Overall Grade", "Ranking"], [[50, obtained, `${studentPercentage(student)}%`, grade(studentPercentage(student)), rank == null ? "—" : rank]])}<h2>RESULT</h2><p><b>Overall Performance:</b> ${escapeHtml(performanceStatus(studentPercentage(student)))}</p><p><b>Class / Section Rank:</b> ${rank == null ? "__________________________" : rank}</p><h2>TEACHER / TRAINER REMARKS</h2><p style="height:70px;border-bottom:1px solid #d8dee9"></p>`; }
