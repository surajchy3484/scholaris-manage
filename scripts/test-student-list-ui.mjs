...
const handlers=await load('src/lib/performance.functions.ts',{'@tanstack/react-start':{createServerFn},'./app-access.server':{requirePermission:async(token,module,requested)=>{action=requested;if(!allowed)throw new Error('Denied');return {role:'trainer',schoolIds:[],allSchools:false};},adminDb:async()=>{calls++;return {rpc:async()=>({data:{rows:[],total:0},error:null})};}},'./access-control':{canSeeSchool:()=>visible}});
