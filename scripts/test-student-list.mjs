...
await db.exec(
  await fs.readFile("supabase/migrations/20260925090000_performance_paging.sql", "utf8"),
);
await db.exec(
  await fs.readFile("supabase/migrations/20260925133000_student_details_list.sql", "utf8"),
);
...
const student = (
  await db.query("SELECT id FROM students WHERE school_id=$1 AND roll_number='1'", [school])
).rows[0].id;
await db.query(
  `INSERT INTO exam_scores(school_id,student_id,exam_type,score,updated_at) VALUES ($1,$2,'ICA',0,'2026-01-01'),($1,$2,'IMF',60,'2026-01-01'),($1,$2,'MCA',80,'2026-02-01'),($1,$2,'FCA',90,'2026-01-01')`,
  [school, student],
);
await db.query(
  `INSERT INTO attendance(student_id,school_id,status) VALUES ($1,$2,'present'),($1,$2,'absent')`,
  [student, school],
);
const page = async (options = {}) =>
  (
    await db.query(
      "SELECT student_details_page(p_school:=$1,p_page:=$2,p_search:=$3,p_attendance:=$4,p_exam:=$5,p_status:=$6,p_min:=$7,p_max:=$8) r",
      [
        school,
        options.page ?? 0,
        options.search ?? "",
        options.attendance ?? "all",
        options.exam ?? "ICA",
        options.status ?? "all",
        options.min ?? null,
        options.max ?? null,
...
await db.query(
  "INSERT INTO exam_scores(school_id,student_id,exam_type,score,updated_at) VALUES ($1,$2,'ATTENDANCE',100,'2026-01-01')",
  [school, student],
);
...
await assert.rejects(() =>
  db.query("SELECT delete_student_details($1,$2::uuid[])", [school, [student]]),
);
...
assert.equal(
  (await db.query("SELECT count(*)::int n FROM exam_scores WHERE student_id=$1", [student])).rows[0]
    .n,
  0,
);
await db.close();
console.log(
  "PASS: 10,000 students, page boundaries, zero/missing scores, IMF fallback and MCA priority, attendance and override filters, escaped search, school isolation, RPC role denial and atomic score cleanup",
);
